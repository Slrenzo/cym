...
class StatsService
{
    ...
    public function getMaxHumeur($pdo, $id) {
        $req =$pdo->prepare("SELECT Humeur_Libelle, COUNT(Humeur_Libelle) as compteur, MAX(Humeur_Emoji) as Humeur_Emoji from humeur join user ON user.User_ID = humeur.CODE_USER WHERE CODE_User = :id GROUP BY Humeur_Libelle ORDER BY compteur DESC LIMIT 1");
        $req->execute(['id'=>$id]);
        if($req->rowCount() == 0) {
    ...
    public function verifHumeurEstPresente($pdo, $startDate, $endDate, $humeur,$id) {
        $req =$pdo->prepare("SELECT * from humeur join user ON user.User_ID = humeur.CODE_USER WHERE CODE_User = :id AND Humeur_Libelle = :humeur AND Humeur_Time BETWEEN :startDate AND :endDate");
        $req->bindParam('id', $id);
        $req->bindParam('startDate', $startDate);
        $req->bindParam('endDate', $endDate);
    ...
    public function verifIsThere($pdo, $startDate, $endDate, $id) {
        $req =$pdo->prepare("SELECT * from humeur join user ON user.User_ID = humeur.CODE_USER WHERE CODE_User = :id AND Humeur_Time BETWEEN :startDate AND :endDate");
        $req->bindParam('id', $id);
        $req->bindParam('startDate', $startDate);
        $req->bindParam('endDate', $endDate);
        $req->execute();
    ...
    public function getMostUsed($pdo, $startDate, $endDate, $humeurs, $id) {
        ...
        if ($humeurs == "TOUS") {
            ...
            $req = $pdo->prepare ("SELECT COUNT(`Humeur_Libelle`) AS 'NB_Humeur', `Humeur_Emoji` AS 'Emoji' FROM `humeur` WHERE `CODE_User` = :id AND `Humeur_Libelle` = :libelle AND `Humeur_Time` <= :endDate AND `Humeur_Time` >= :startDate GROUP BY `Humeur_Emoji`");
    ...
    public function getHumeurByTime($pdo, $startDate, $endDate, $humeurs, $id) {
        $req = $pdo->prepare("SELECT count(*) as nombreHumeur, MAX(Humeur_Libelle) as Humeur_Libelle, DATE_FORMAT(Humeur_Time, '%d/%m/%Y') as Date from humeur where code_User=:id AND Humeur_Libelle = :libelle and Humeur_Time BETWEEN :startDate AND :endDate GROUP BY Date");
        $req->execute(['id'=>$id, 'libelle'=>$humeurs, 'startDate'=>$startDate, 'endDate'=>$endDate]);
        return $req;
    }
